...
async function showSetTimerModalDialog(title, message, okButtonText) {
    ...
    return new Promise((resolve) => {
        ...
        const timerDialogCard = document.createElement("div");
        timerDialogCard.className = "set-timer-modal-card-dialog";

        const timerDialogCardHeader = document.createElement("div");
        timerDialogCardHeader.className = "set-timer-modal-header-dialog";
        const timerDialogCardHeaderTitle = document.createElement("h3");
        timerDialogCardHeaderTitle.className = "set-timer-modal-title-dialog";
        timerDialogCardHeaderTitle.textContent = title;
        timerDialogCardHeader.appendChild(timerDialogCardHeaderTitle);

        const timerDialogCardBody = document.createElement("div");
        timerDialogCardBody.className = "set-timer-modal-body-dialog";
        if (message) {
          const timerDialogCardBodyMessage = document.createElement("p");
          timerDialogCardBodyMessage.className = "set-timer-modal-message-dialog";
          timerDialogCardBodyMessage.innerHTML = message;
          timerDialogCardBody.appendChild(timerDialogCardBodyMessage);
        }

        const timerInput = document.createElement("form");
        for (let i = 0; i < timerTimeList.length; i++) {
          const timeOption = timerTimeList[i];
          const optionId = `timerOption${i}`;
          const optionHtml = `
            <div class="form-check">
              <input
                ...
                type="radio"
                name="radioDefault"
                id="${optionId}" ${timerTimeList[i].value === state.player_settings.sleepTimerDuration ? "checked" : ""}>
              <label class="form-check-label" for="${optionId}">
                ${timeOption.label}
              </label>
            </div>
          `;
          timerInput.insertAdjacentHTML("beforeend", optionHtml);
        }
        ...
        timerDialogCardBody.appendChild(timerInput);

        const timerDialogCardFooter = document.createElement("div");
        timerDialogCardFooter.className = "set-timer-modal-footer-dialog";
        const timerDialogCardFooterOkButton = document.createElement("input");
        timerDialogCardFooterOkButton.type = "submit";
        timerDialogCardFooterOkButton.classList.add("btn", "btn-primary", "set-timer-modal-ok-btn-dialog");
        timerDialogCardFooterOkButton.value = okButtonText;
        const timerDialogCardFooterCancelButton = document.createElement("button");
        timerDialogCardFooterCancelButton.type = "button";
        timerDialogCardFooterCancelButton.classList.add("btn", "btn-secondary", "set-timer-modal-cancel-btn-dialog");
        timerDialogCardFooterCancelButton.textContent = "İptal";
        timerDialogCardFooter.appendChild(timerDialogCardFooterOkButton);
        timerDialogCardFooter.appendChild(timerDialogCardFooterCancelButton);

        const cleanup = () => {
            timerDialog.close();
            timerDialogCardFooterOkButton.removeEventListener("click", onOk);
            timerDialogCardFooterCancelButton.removeEventListener("click", onCancel);
            body.removeChild(timerDialog);
        };
        ...
        const onOk = () => {
            ...
            if (minutes === 0) {
                cancelSleepTimer(sleepTimerTimeoutId, sleepTimerIntervalId);
                resolve(minutes);
                cleanup();
                return;
            }
        ...
        timerDialogCardFooterOkButton.addEventListener("click", onOk);
        timerDialogCardFooterCancelButton.addEventListener("click", onCancel);

        timerDialogCard.appendChild(timerDialogCardHeader);
        timerDialogCard.appendChild(timerDialogCardBody);
        timerDialogCard.appendChild(timerDialogCardFooter);
        timerDialog.appendChild(timerDialogCard);
        body.appendChild(timerDialog);
...
async function showSetTimerModalDialog2(title, message, okButtonText) {
    ...
    return new Promise((resolve) => {
        ...
        const onOk = () => {
            ...
            if (minutes === 0) {
                cancelSleepTimer2(timerDisplay);
                resolve(minutes);
                cleanup();
                return;
            }
  ...
  if (state.player_settings.sleepTimerDuration > 0) {
    ...
    timeOutId = setTimeout(() => {
      if (vp) {
        vp.pause();
        cancelSleepTimer(sleepTimerTimeoutId, sleepTimerIntervalId);
        timerDisplay.countdownIsActive = false;
    ...
    intervalId = setInterval(() => {
      const remainingTime = Math.max(0, durationMs - (Date.now() - startTime));
...
function startSleepTimer2(vp) {
  ...
  if (state.player_settings.sleepTimerDuration > 0) {
    ...
    timerDisplay.target = durationMs;
    timerDisplay.countDown();
  }
}

function cancelSleepTimer(timeOutId, intervalId) {
  if (timeOutId) {
  ...
  document.getElementById("sleep-timer-countdown-hours").textContent = "";
  document.getElementById("sleep-timer-countdown-minutes").textContent = "";
  document.getElementById("sleep-timer-countdown-seconds").textContent = "";
  state.player_settings.sleepTimerDuration = 0;
  saveState();
}

function cancelSleepTimer2(timerDisplay) {
  if (timerDisplay.timeOutId) {
    clearTimeout(timerDisplay.timeOutId);
  }
  if (timerDisplay.ref) {
    cancelAnimationFrame(timerDisplay.ref);
  }
  timerDisplay.countdownIsActive = false;
  timerDisplay.target = null;
  timerDisplay.ref = null;
  timerDisplay.timeOutId = null;
  timerDisplay.updateDisplay("", "", "");
  // document.getElementById("sleep-timer-countdown-hours").textContent = "";
  // document.getElementById("sleep-timer-countdown-minutes").textContent = "";
  // document.getElementById("sleep-timer-countdown-seconds").textContent = "";
