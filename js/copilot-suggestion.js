// Ensure video player only shows/plays in fullscreen mode
function showVideoPlayerFullscreen(videoUrl) {
...
// Listen for Escape (desktop) and fullscreenchange
...
// Android back button: use popstate (PWA/standalone)
...
removeAllFilesBtn.addEventListener("click", async () => {
	// if (
	// 	state.profiles.length === 0 && state.profiles.every(profile => !profile.videos 
	// 		|| profile.videos.length === 0 
	// 		|| profile.videos.every(video => !video.src || video.src === ""))
	// ) {
	if (
		state.profiles.length === 1 && state.profiles[0].originalName === "Varsayılan" &&
		state.profiles[0].videos.every(video => !video.src || video.src === "")
	) {
		...
		alertDiv.textContent = "Silinecek dosya veya profil bulunamadı.";
...
document.addEventListener("DOMContentLoaded", async () => {
	...
	storageInfo();
	vp = videojs("vp", {
		controls: state.player_settings.showVideoControls,
		fluid: true,
		autoplay: false,
		preload: 'auto',
		loadingSpinner: true,
	...
	for (const video of currentProfile.videos) {
		...
		videoUploadInput.addEventListener("change", async (e) => {
			...
			if (checkedFile && checkedFile.size === file.size && checkedFile.name === file.name) {
				...
				rowContainer.insertAdjacentElement("beforebegin", duplicateFileAlert);
				...
				const timeoutId = setTimeout(() => {
					if (rowContainer.parentElement.contains(duplicateFileAlert)) {
						rowContainer.parentElement.removeChild(duplicateFileAlert);
			...
			const thumbnailBlobData = await generateThumbnail(videoUrl).then(async thumbnailUrl => {
				...
				const message = "Video için poster oluşturulurken bir hata oluştu. Video biçimi desteklenmiyor olabilir. Lütfen farklı bir video dosyası seçin.";
				showNotificationModalDialog("Dikkat", message, "Tamam");
				console.error("Error generating thumbnail blob: ", err);
				return null;
			});
			...
			try {
				...
				if (err.name === 'AbortError') {
					...
					console.log(err);
					message = `
						Aşağıdaki video dosyasının yüklemesi iptal edildi:
						<ul>
							<li>${file.name}</li>
						</ul>
					`;
			...
			if (uploadError) {
				...
				videoIndicator.style.backgroundImage = "none";
				queuedOPFSOperationsCount--;
				const removedFromGlobal = activeUploadsGlobal.delete(video.id);
				const removedFromSet = activeUploadsSet.delete(video.originalFileName);
				console.log(`Removed ${video.storedFileName} from activeUploadsGlobal: ${removedFromGlobal}`);
				console.log(`Removed ${video.storedFileName} from activeUploadsSet: ${removedFromSet}`);
...
async function renderVideoList() {
	...
	for (const video of currentProfile.videos) {
		...
		videoListItem.addEventListener("click", async () => {
			...
			showVideoPlayerFullscreen(video.src);
			state.currentlyPlayingVideoId = video.id;
			saveState();
		});
...
function storageInfo() {
	navigator.storage.estimate().then(({quota, usage}) => {
		...
		const quotaInGiB = `${(quota / 1073741824).toFixed(2)}`;
		OPFSDiskUsage.textContent = `DİSK: Kullanılan ${usageInGiB < 1 ? `${usageInMiB} MiB` : `${usageInGiB} GiB`} Kalan ${`${remainingInGib} GiB`}; Kota ${`${quotaInGiB} GiB`}`;
	});
}
...
function storageInfoDebug() {
	navigator.storage.estimate().then(({quota, usage}) => {
		...
		const quotaInGiB = `${(quota / 1073741824).toFixed(2)}`;
		console.log(usage);
		console.log(`DİSK: Kullanılan ${usageInGiB < 1 ? `${usageInMiB} MiB` : `${usageInGiB} GiB`} Kalan ${`${remainingInGib} GiB`}; Kota ${`${quotaInGiB} GiB`}`);
	});
}
